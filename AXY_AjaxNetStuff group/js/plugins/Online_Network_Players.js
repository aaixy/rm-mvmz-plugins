...
var NetworkPlayerPartyLeader = '';
var NetworkPlayerPartyLength = 0;
...
Game_Network.prototype.connectSocketsAfterLogin = function(){
	...
	socket.on('NetworkPlayersXY', function(data){
		...
		NetworkPlayerPartyLeader = data.partyLeader;
		NetworkPlayerPartyLength = data.partyLength;
		...
		//networkMapEvents[player].setOpacity(100);
				...
				if (direction > 0) {
					//Send x,y info to server
						...
						socket.emit('DestinationXY', {
							...
							partyLeader: $gameActors._data[$gameParty._actors[0]]._name,
							partyLength: $gameParty._actors.length
						});
						/*console.log(this._characterName);
						console.log($gamePlayer);
						console.log($gameParty);
						console.log($gameActors);
						console.log($gameActors._data[$gameParty._actors[this._characterIndex]]._name);
...
Game_NetworkPlayer.prototype.event = function() {
    $networkPlayerMap.events[NetPlayerEventID].pages[0].list[0].parameters[0] = '<Mini Label: '+NetworkPlayerPartyLeader+'('+NetworkPlayerPartyLength+')>';
	//console.log($networkPlayerMap.events[NetPlayerEventID]);

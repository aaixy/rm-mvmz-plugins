...
Spriteset_Map.prototype.createArea = function () {
    ...
    for (var i = 0; i < $gameMap.width() * $gameMap.height(); i++) {
        ...
        if (data - Lagomoro.MapArea.start >= max) {
            max = data - Lagomoro.MapArea.start;
        }
    }
    ...
    for (var i = 0; i <= datamax; i++) {
        ...
        for (var a = 0; a < $gameMap.width(); a++) {
            for (var b = 0; b < $gameMap.height(); b++) {
                ...
                if (data - Lagomoro.MapArea.start === i) {
                    if (a < slice.x1 && slice.x1 !== 0) {
                        slice.x1 = a;
                    }
                    if (b < slice.y1 && slice.y1 !== 0) {
                        slice.y1 = b;
                    }
                    if (a > slice.x2) {
                        slice.x2 = a;
        ...
        this._Areasite[i] = {
            x: slice.x1 * $gameMap.tileWidth(),
            y: slice.y1 * $gameMap.tileHeight()
        };
